
Feature: Tests for the JSON transform

        @json
        Scenario: Karate map operation
            * def fun = function(x){ return x * x }
            * def list = [1, 2, 3]
            * def result = karate.map(list, fun)
            * print result
              And match result == [1, 4, 9]

        @json
        Scenario: Convert an array into a different shape
            * def before = [{ foo: 1 }, { foo: 2 }, { foo: 3 }]
            * def fun = function(x){ return { bar: x.foo } }
            * def after = karate.map(before, fun)
              And match after == [{ bar: 1 }, { bar: 2 }, { bar: 3 }]

        @json
        Scenario: Convert array of primitives into array of objects
            * def list = [ 'Bob', 'Wild', 'Nyan' ]
            * def data = karate.mapWithKey(list, 'name')
              And match data == [{ name: 'Bob' }, { name: 'Wild' }, { name: 'Nyan' }]

        @json
        Scenario: Karate filter operation
            * def fun = function(x){ return x % 2 == 0 }
            * def list = [1, 2, 3, 4]
            * def res = karate.filter(list, fun)
              And match res == [2, 4]

        @json
        Scenario: forEach works even on object key-values, not just arrays
            * def keys = []
            * def vals = []
            * def idxs = []
            * def fun = 
            """
            function(x, y, i) { 
            karate.appendTo(keys, x); 
            karate.appendTo(vals, y); 
            karate.appendTo(idxs, i); 
            }
            """
            * def map = { a: 2, b: 4, c: 6 }
            * karate.forEach(map, fun)
              And match keys == ['a', 'b', 'c']
              And match vals == [2, 4, 6]
              And match idxs == [0, 1, 2]

        @json
        Scenario: FilterKeys
            * def schema = { a: '#string', b: '#number', c: '#boolean' }
            * def response = { a: 'x', c: true }
            # very useful for validating a response against a schema "super-set"
              And match response == karate.filterKeys(schema, response)
              And match karate.filterKeys(response, 'b', 'c') == { c: true }
              And match karate.filterKeys(response, ['a', 'b']) == { a: 'x' }

        @json
        Scenario: merge
            * def foo = { a: 1 }
            * def bar = karate.merge(foo, { b: 2 })
              And match bar == { a: 1, b: 2 }

        @json
        Scenario: append
            * def foo = [{ a: 1 }]
            * def bar = karate.append(foo, { b: 2 })
              And match bar == [{ a: 1 }, { b: 2 }]

        @json
        Scenario: number to string and string to number
            * def foo = 10
            * string json = {"bar": #(foo + '')}
            * print json
              And match json == {"bar": "10"}

            * def boo = '10'
            * string jsons = { bar: #(parseInt(boo)) }
            * print jsons
            * match jsons == '{"bar":10.0}'

            * string jsons = { bar: #(boo * 1) }
            * print jsons
            * match jsons == '{"bar":10.0}'

            * string jsons = { bar: #(~~parseInt(boo)) }
            * print jsons
            * match jsons == '{"bar":10}'